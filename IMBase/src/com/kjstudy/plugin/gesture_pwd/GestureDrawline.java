...
public class GestureDrawline extends View {
	...
	private int mDefaultLine = Color.rgb(255, 255, 255);
	private int mErrLine = Color.rgb(154, 7, 21);
	...
	public void setErrLine(int mErrLine) {
		this.mErrLine = mErrLine;
	}
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_MOVE:
			...
			if (pointAt == null || currentPoint.equals(pointAt)
					|| PointState.SELECTED == pointAt.getPointState()) {
				...
				if (betweenPoint != null
						&& PointState.SELECTED != betweenPoint.getPointState()) {
					// 存在中间点并且没有被选中
	...
	private void drawErrorPathTip() {
		canvas.drawColor(Color.TRANSPARENT, PorterDuff.Mode.CLEAR);
		paint.setColor(mErrLine);// 设置默认线路颜色
		for (Pair<GesturePoint, GesturePoint> pair : lineList) {
