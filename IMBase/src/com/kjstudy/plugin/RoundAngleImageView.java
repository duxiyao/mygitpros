...
public class RoundAngleImageView extends ImageView {
	...
	private void init(Context context, AttributeSet attrs) {
		...
		TypedArray a = context.obtainStyledAttributes(attrs,
				R.styleable.AngleImgV);
		int angle = a.getInt(R.styleable.AngleImgV_angle, 5);
		a.recycle();

		roundHeight = roundWidth = angle;
		paint = new Paint();
	...
	public void draw(Canvas canvas) {
		...
		// super.draw(canvas);
		//
		// canvas.drawPath(getRoundCornerPath(roundWidth, roundHeight), paint);
		//
		canvas.drawBitmap(bitmap, 0, 0, paint2);
		bitmap.recycle();
	}
	...
	public Path getRoundCornerPath(int roundWidth, int roundHeight) {
		Path path = new Path();
		path.moveTo(0, roundHeight);
		path.lineTo(0, 0);
		path.lineTo(roundWidth, 0);
