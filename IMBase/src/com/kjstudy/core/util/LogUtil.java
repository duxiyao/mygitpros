...
public class LogUtil {
	...
	public static File creatFile(String filename) {
		String root_path = FileAccessor.CRASH_PATH;
		File dir = new File(root_path);
		try {
			if (!dir.exists()) {
				dir.mkdirs();
			}
			File f = new File(root_path + File.separator + filename);
			if (!f.exists()) {
				f.createNewFile();
			}
			return f;
