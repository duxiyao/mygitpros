...
import org.myframe.database.CursorHelper;
import org.myframe.database.DaoConfig;
import org.myframe.database.DbModel;
import org.myframe.database.OneToManyLazyLoader;
import org.myframe.database.SqlBuilder;
import org.myframe.database.SqlInfo;
import org.myframe.database.utils.KeyValue;
import org.myframe.database.utils.ManyToOne;
import org.myframe.database.utils.OneToMany;
import org.myframe.database.utils.TableInfo;
...
public class MDB {
	...
	/** 用于保存所有数据库 */
	private static HashMap<String, MDB> daoMap = new HashMap<String, MDB>();

	private SQLiteDatabase db;
	private final DaoConfig config;

	private MDB(DaoConfig config) {
		if (config == null) {
	...
	private synchronized static MDB getInstance(DaoConfig daoConfig) {
		MDB dao = daoMap.get(daoConfig.getDbName());
		if (dao == null) {
			dao = new MDB(daoConfig);
	 ...
	 */
	public static MDB create(boolean isDebug) {
	 ...
	 * @param context
	 */
	public static MDB create(Context context) {
	 ...
	 *            是否是debug模式（debug模式进行数据库操作的时候将会打印sql语句）
	 */
	public static MDB create(Context context, boolean isDebug) {
		DaoConfig config = new DaoConfig();
		config.setContext(context);
	 ...
	 *            数据库名称
	 */
	public static MDB create(Context context, String dbName) {
		DaoConfig config = new DaoConfig();
		config.setContext(context);
	 ...
	 */
	public static MDB create(Context context, String dbName, boolean isDebug) {
		DaoConfig config = new DaoConfig();
		config.setContext(context);
		config.setDbName(dbName);
		config.setDebug(isDebug);
	 ...
	 *            数据库名称
	 */
	public static MDB create(Context context, String targetDirectory,
			String dbName) {
		DaoConfig config = new DaoConfig();
		config.setContext(context);
		config.setDbName(dbName);
	 ...
	 * @return
	 */
	public static MDB create(DaoConfig daoConfig) {
		...
		if (cursor != null) {
			while (cursor.moveToNext()) {
				...
				try {
					...
					MLoger.debug(getClass().getName() + e.getMessage());
				}
			}
		}
		if (cursor != null) {
	...
	private void exeSqlInfo(SqlInfo sqlInfo) {
		if (sqlInfo != null) {
			...
			MLoger.debug(getClass().getName() + "sava error:sqlInfo is null");
		}
	}
	 ...
	 * @return
	 */
	public boolean isExists(Class<?> clazz, String strWhere) {
		checkTableExist(clazz);
		String strSQL = SqlBuilder.getSelectSQLByWhere(clazz, strWhere);
		debugSql(strSQL);
		Cursor cursor = db.rawQuery(strSQL, null);
		try {
	...
	private void checkTableExist(Class<?> clazz) {
		if (!tableIsExist(TableInfo.get(clazz))) {
			...
			checkTableStructure(clazz);
		}
	}

	private void checkTableStructure(Class<?> clazz) {
		...
		try {
			...
			if (cursor != null) {
				...
				for (String colName : table.propertyMap.keySet()) {
